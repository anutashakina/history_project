...
const createCustomIcon = (isActive: boolean) => {
    const iconMarkup = renderToStaticMarkup(
        <div className={`relative ${isActive ? "text-[#C19A6B]" : "text-[#8B7D6B]"}`}>
            <MapPin className="w-10 h-10 drop-shadow-xl" fill={isActive ? "#C19A6B" : "#6B5D54"} strokeWidth={1.5} />
...
export const InteractiveMap: React.FC<{ markers: any[]; onMarkerClick?: (id: number) => void }> = ({ markers, onMarkerClick }) => {
    ...
    return (
        <div className="relative w-full h-full bg-[#E8E3D6] rounded-2xl overflow-hidden border-4 border-[#6B5D54]/30 shadow-2xl">
            <div className="absolute inset-0 pointer-events-none z-[1000] mix-blend-multiply opacity-40" style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg width='100' height='100' viewBox='0 0 100 100' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='noise'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.8' numOctaves='4' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100' height='100' filter='url(%23noise)' opacity='0.1'/%3E%3C/svg%3E")` }} />
            ...
            <div className="absolute top-6 left-8 z-[1001] pointer-events-none font-serif">
                <h2 className="text-[#4A3728] text-sm uppercase tracking-[0.3em] font-bold opacity-80">Цифровой атлас</h2>
            ...
            <div className="absolute bottom-6 right-8 z-[1001] pointer-events-none font-serif text-right text-[#4A3728]/60 text-[10px] uppercase tracking-widest">
                <span>Масштаб: 1 : 50 000</span><br /><span>Архивные данные 1941—1944 гг.</span>
            </div>
        </div>
    );
};
